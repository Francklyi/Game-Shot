...
const vec3 _NoiseAnim = vec3(0.0, -1.0, 0);
...
{	
	...
	d += fbm(p*_NoiseFreq + _NoiseAnim*time_v) * _NoiseAmp;
	return d;
}
...
vec4 gradient(float x)
{
	...
	const vec4 c0 = vec4(2.0, 2.0, 1.0, 1.0);	// yellow
	const vec4 c1 = vec4(1.0, 0.0, 0.0, 1.0);	// red
	const vec4 c2 = vec4(0.0, 0.0, 0.0, 0.0); 	// black
	const vec4 c3 = vec4(0.0, 0.5, 1.0, 0.5); 	// blue
...
void main(void)
{
	vec2 iResolution=vec2(1196.0,768.0);
    vec2 p = (gl_FragCoord.xy / iResolution.xy)*2.0-1.0;
    p.x *= iResolution.x/ iResolution.y;
	
    float rotx = (0.0 / iResolution.y)*4.0;
    float roty = -(0.0 / iResolution.x)*4.0;
