...
int Crystal::crystalSum=0;
Crystal::Crystal():theta(0),visable(true),
px(10),py(5),pz(10)
,R(6)
,isClose(false)
,Graph()
...
void Crystal::update(LeadRole *pg)
{
	//theta++;
	...
	if(isClose)
	{
		if(!(visable=!isCapture(pg)))
			{
				//pMjni->captureCrystal();
				//pGame->numCrystalGet++;
				pGame->isAddCrystal=true;
				return;
			}
...
void Crystal::updateAfterTransform()
{

}

void Crystal::updateAfterDraw()
{

}

void Crystal::setVisable(bool visable_)
{
	visable=visable_;
}

bool Crystal::isVisable()
{
	return visable;
}

double Crystal::random(int seed,double start, double end)
{
  srand(seed);
  return start+(end-start)*rand()/(RAND_MAX + 1.0);
}

bool Crystal::getIsClose(LeadRole *pg)
{
	pos0=pTransform->pTOmatrix->mMatrixQueue.back();
	pos1=pg->pTransform->pTOmatrix->mMatrixQueue.back();
...
bool Crystal::isCapture(LeadRole *pg)
{
